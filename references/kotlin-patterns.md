...
- Use `data class` for value types, `sealed class`/`sealed
  interface` for state
...
data class UserProfile(
    ...
    override fun redacted() =
        "UserProfile(id=$id, email=[REDACTED], name=[REDACTED])"
    override fun toString() = redacted() // Prevent accidental
                                         // PII logging
}
```
